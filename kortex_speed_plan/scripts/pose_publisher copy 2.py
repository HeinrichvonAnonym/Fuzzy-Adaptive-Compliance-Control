...
class GraspTrajectoryPublisher:
    ...
    def setup_poses(self):
        ...
        self.start_pose.position.x = 0.2
        self.start_pose.position.y = -0.2
        self.start_pose.position.z = 0.55
        self.start_pose.orientation = self.pose.orientation  # 已经设置为竖直向下
        
        # 保存起点姿态的拷贝，用于返回起点
        self.return_start_pose = Pose()
        self.return_start_pose.position.x = self.start_pose.position.x
        self.return_start_pose.position.y = self.start_pose.position.y
        self.return_start_pose.position.z = self.start_pose.position.z
        ...
        self.place_pose.position.z = 0.6
        ...
        # 设置默认抓取位置
        self.setup_default_object_pose()
    ...
    def setup_default_object_pose(self):
        """设置默认的抓取位置"""
        # 默认抓取位置: (0.35, -0.35, 0.42)，保持竖直向下的姿态并绕z轴旋转90度
        self.object_pose = Pose()
        self.object_pose.position.x = 0.4
        self.object_pose.position.y = -0.4
        self.object_pose.position.z = 0.42
    ...
    def current_pose_callback(self, pose_array_msg):
        ...
        if len(pose_array_msg.poses) >= 2:
            ...
            # 检查是否已经到达当前目标点
            if not self.target_reached and not self.trajectory_completed:
    ...
    def check_if_target_reached(self):
        ...
        # 确定当前目标
        target_pose = None
        if self.current_target == "start" and self.start_pose:
            target_pose = self.start_pose
            target_desc = "起点"
            target_index = 0
        elif self.current_target == "object" and self.object_pose:
            target_pose = self.object_pose
            target_desc = "抓取位置"
            target_index = 1
        elif self.current_target == "place" and self.place_pose:
            target_pose = self.place_pose
            target_desc = "放置位置"
            target_index = 2
        elif self.current_target == "return_start" and self.return_start_pose:
    ...
    def get_all_poses(self):
        ...
        poses = []
        if self.start_pose:
            poses.append(self.start_pose)
        if self.object_pose:
            poses.append(self.object_pose)
        if self.place_pose:
            poses.append(self.place_pose)
        if self.return_start_pose:
            poses.append(self.return_start_pose)
        return poses
